...
!-------------------- creating output Files
      if(intfoc.ne.0.and.myid.eq.0) then
        open(FILEEN,file='requester-data/energy',status='replace')
        close(FILEEN)
        open(FILEPP,file='requester-data/pprobe',status='replace')
        close(FILEPP)
        open(FILEPB,file='requester-data/pbody',status='replace')
        close(FILEPB)
        open(FILEIC,file='requester-data/icur',status='replace')
        close(FILEIC)
        open(FILEOC,file='requester-data/ocur',status='replace')
        close(FILEOC)
...
!        close(92)
        open(93, file='requester-data/chgacm1',status='replace')
        close(93)
        open(94, file='requester-data/chgacm2',status='replace')
        close(94)
!        open(95, file='chgmov', status='replace')
!        close(95)
        open(96, file='requester-data/seyield', status='replace')
        close(96)
