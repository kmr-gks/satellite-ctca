...
!
  interface wrt3mp
    module procedure wrt3i_mp,wrt3r_mp,wrt3d_mp
  end interface wrt3mp

  interface wrt1vmp
    module procedure wrt1iv_mp,wrt1rv_mp,wrt1dv_mp
  end interface wrt1vmp
...
!
!      subroutine wrt3d_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
!        use HDF5
!        implicit none
!        integer(kind=4),parameter :: rank=3
!
...
!
  ...
  subroutine wrt1i_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    integer(kind=4),intent(in) :: data(-tab:dsize(1)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
...
!
    ...
    call h5dcreate_f(f_id,dsname,H5T_NATIVE_INTEGER,filespace,d_id,status)
    ...
    call h5dwrite_f(d_id, H5T_NATIVE_INTEGER, &
   &                data(lb(1):ub(1):stride(1)), &
   &                all_data_size, istat, &
...
!
    return
  end subroutine wrt1i_mp


  subroutine wrt1r_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    real(kind=4),intent(in) :: data(-tab:dsize(1)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
    return
  end subroutine wrt1r_mp


  subroutine wrt1d_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    real(kind=8),intent(in) :: data(-tab:dsize(1)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
    ...
    call h5dwrite_f(d_id, H5T_NATIVE_DOUBLE, &
   &                data(lb(1):ub(1):stride(1)), &
   &                all_data_size, istat, &
   &                file_space_id=filespace, mem_space_id=memspace, xfer_prp=p_id)
...
!
    return
  end subroutine wrt1d_mp


  subroutine wrt2i_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=2
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    integer(kind=4),intent(in) :: data(-tab:dsize(1)-1+tab,-tab:dsize(2)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
   ...
   &                                     lb(2):ub(2):stride(2)))
    ...
    call h5dwrite_f(d_id, H5T_NATIVE_INTEGER, &
   &                data(lb(1):ub(1):stride(1),lb(2):ub(2):stride(2)), &
   &                all_data_size, istat, &
   &                file_space_id=filespace, mem_space_id=memspace, xfer_prp=p_id)
...
!
    return
  end subroutine wrt2i_mp


  subroutine wrt2r_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=2
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    real(kind=4),intent(in) :: data(-tab:dsize(1)-1+tab,-tab:dsize(2)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
    return
  end subroutine wrt2r_mp


  subroutine wrt2d_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=2
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    real(kind=8),intent(in) :: data(-tab:dsize(1)-1+tab,-tab:dsize(2)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
    return
  end subroutine wrt2d_mp


  subroutine wrt3i_mp(f_id,dsname,myid,nodes,dsize,tab,data,istat,status,odoms)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=3
!
    ...
    integer(kind=HSIZE_T),intent(in) :: dsize(rank)
    integer(kind=HSIZE_T),intent(in) :: tab
    integer(kind=4),intent(in) :: data(-tab:dsize(1)-1+tab,-tab:dsize(2)-1+tab,-tab:dsize(3)-1+tab)
    integer(kind=4),intent(out) :: istat, status
    integer(kind=HSIZE_T),intent(in),optional :: odoms(3,rank)
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
...
!
  ...
  subroutine wrt1iv_mp(f_id,dsname,myid,ldsize,gdsize,offset,data,istat,status)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    character(len=*),intent(in) :: dsname
    integer(kind=4),intent(in) :: myid
    integer(kind=HSIZE_T),intent(in) :: gdsize(rank)
    integer(kind=HSIZE_T),intent(in) :: ldsize(rank)
    integer(kind=HSIZE_T),intent(in) :: offset(rank)
    integer(kind=4),intent(in) :: data(ldsize(1))
    integer(kind=4),intent(out) :: istat, status
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
!
    ...
    call h5screate_simple_f(rank,gdsize(1:rank),filespace,status)
    call h5dcreate_f(f_id,dsname,H5T_NATIVE_INTEGER,filespace,d_id,status)
    call h5sclose_f(filespace,status)

    call h5screate_simple_f(rank, ldsize(1:rank), memspace, status)
    call h5dget_space_f(d_id, filespace, status)
    call h5sselect_hyperslab_f(filespace, H5S_SELECT_SET_F, offset(1:rank), ldsize(1:rank), status)
...
!
    return
  end subroutine wrt1iv_mp


  subroutine wrt1rv_mp(f_id,dsname,myid,ldsize,gdsize,offset,data,istat,status)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    integer(kind=HSIZE_T),intent(in) :: ldsize(rank)
    integer(kind=HSIZE_T),intent(in) :: offset(rank)
    real(kind=4),intent(in) :: data(ldsize(1))
    integer(kind=4),intent(out) :: istat, status
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
!
...
!
    return
  end subroutine wrt1rv_mp


  subroutine wrt1dv_mp(f_id,dsname,myid,ldsize,gdsize,offset,data,istat,status)
    use HDF5
    implicit none
    integer(kind=4),parameter :: rank=1
!
    ...
    integer(kind=HSIZE_T),intent(in) :: ldsize(rank)
    integer(kind=HSIZE_T),intent(in) :: offset(rank)
    real(kind=8),intent(in) :: data(ldsize(1))
    integer(kind=4),intent(out) :: istat, status
!
    integer(kind=HID_T) :: s_id, d_id, filespace, memspace, p_id
!
