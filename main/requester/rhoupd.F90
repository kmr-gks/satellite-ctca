...
!-------------------- continuity equation for charge
      ...
      do j=0,yu
      do i=0,xu
        rhobk(1,i,j,k,ps) = rhobk(1,i,j,k,ps) &
       &                  - ( aj(JX,i,j,k,2+ps) - aj(JX,i-1,j,k,2+ps) &
       &                    + aj(JY,i,j,k,2+ps) - aj(JY,i,j-1,k,2+ps) &
       &                    + aj(JZ,i,j,k,2+ps) - aj(JZ,i,j,k-1,2+ps) )*2.0d0
      end do
      end do
      end do
