...
!             ------- B-components
          ...
          if(nwave.lt.0) then
            ...
            orgb(1:3) = orgwv(1:3,iw)
!           orgb(1) = orgwv(1,iw) -
!           cv*0.5d0*dt*sin(angwv(1,iw))*cos(angwv(2,iw))
!           orgb(2) = orgwv(2,iw) -
!           cv*0.5d0*dt*sin(angwv(1,iw))*sin(angwv(2,iw))
!           orgb(3) = orgwv(3,iw) - cv*0.5d0*dt*cos(angwv(1,iw))
!
            do k=-1,zu-zl+1
            do j=-1,yu-yl+1
...
!!!      call fbound(4)
  ...
  function gssinfld(x,y,z,pw,ld,org,ang)
   ...
   &      + (z - org(3))*cos(ang(1))
    ...
    gssinfld = exp(-displ*displ/pw/pw)*sin(2*pi*displ/ld)
