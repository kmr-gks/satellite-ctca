...
def save_hist2d(df, title, file_name):
	...
	hist_data = df.pivot_table(index='energy(10*log10eV)', columns='time', values='par-count', aggfunc=np.sum, fill_value=0)
	#plot histogram
...
count_min, count_max = df_non0['par-count'].agg(['min', 'max'])
...
df_ele, df_ion = df_all[df_all['species'] == 1], df_all[df_all['species'] == 2]
